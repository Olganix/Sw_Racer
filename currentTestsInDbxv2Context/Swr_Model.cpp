...
|                             Load					                             |
\-------------------------------------------------------------------------------*
bool Swr_Model::Load(const uint8_t *buf, size_t size)
{
	Reset();

	SWR_MODELHeader *hdr = (SWR_MODELHeader *)buf;

	if (size < sizeof(SWR_MODELHeader) || memcmp(hdr->signature, SWR_MODEL_SIGNATURE, 4) != 0)
		return false;
	...
	{
		offset = hdr->offsetSection2 + i * sizeof(SWR_MODEL_Section2);

		Swr_Model_TextureUnit* textUnit = new Swr_Model_TextureUnit();
	...
	{
		offset = hdr->offsetSection1 + i * sizeof(SWR_MODEL_Section1);
...
|                             calculeFilesize		                             |
\-------------------------------------------------------------------------------*
size_t Swr_Model::calculeFilesize(bool withTextureUnits)
{
...
|                             CreateFile			                             |
\-------------------------------------------------------------------------------*
uint8_t* Swr_Model::CreateFile(unsigned int *psize)
{
...
|                             write												 |
\-------------------------------------------------------------------------------*
size_t Swr_Model::write(uint8_t *buf, size_t size)
{
	SWR_MODELHeader* hdr = (SWR_MODELHeader*)buf;
	size_t offset = 0;

	strcpy(&(hdr->signature[0]), SWR_MODEL_SIGNATURE);
	...
	offset = hdr->offsetSection1 + hdr->nbSection1 * sizeof(SWR_MODEL_Section1);
	for (size_t i = 0; i < hdr->nbSection1; i++)
	{
		startSection1Offset = hdr->offsetSection1 + i * sizeof(SWR_MODEL_Section1);
	...
	for (size_t i = 0; i < hdr->nbSection1; i++)
	{
		...
		*((uint32_t*)GetOffsetPtr(buf, startSection1Offset + 0x70, true)) = (mListPart.at(i).mListTextureUnit.size()) ? (offset - startSection1Offset) : 0;		//setup section1->offsetSection2;
		offset += mListPart.at(i).write_TextureUnitPointers(buf + offset, size - offset, hdr->offsetSection2 - (hdr->offsetSection1 + i * sizeof(SWR_MODEL_Section1)) , mListTextureUnit);
	}
	...
	offset = hdr->offsetSection2 + hdr->nbSection2 * sizeof(SWR_MODEL_Section2);
	for (size_t i = 0; i < hdr->nbSection2; i++)
	{
		if ((mListTextureUnit.at(i)->mUseSpeed) || (mListTextureUnit.at(i)->mListKeyframe.size() <=1 ))
			continue;

		startSection2Offset = hdr->offsetSection2 + i * sizeof(SWR_MODEL_Section2);
...
|                             saveXml											 |
\-------------------------------------------------------------------------------*
void Swr_Model::saveXml(string filename)
{
	TiXmlDocument doc;
...
|                             export_Xml					                     |
\-------------------------------------------------------------------------------*
TiXmlElement* Swr_Model::export_Xml()
{
	TiXmlElement* mainNode = new TiXmlElement("Swr_Model");
...
|                             Compile				                             |
\-------------------------------------------------------------------------------*/
bool Swr_Model::Compile(TiXmlDocument *doc, bool big_endian)
{
	/*
	
	TiXmlHandle handle(doc);
	TiXmlElement* rootNode = EMO_BaseFile::FindRoot(&handle, "Swr_Model");
	if (!rootNode)
	{
		LOG_DEBUG("Cannot find\"Swr_Model\" in xml.\n");
		return false;
	}

	TiXmlElement* node;
	TiXmlElement* node_SWR_MODELHeader = rootNode->FirstChildElement("Header");
	if (!node_SWR_MODELHeader)
	{
	...
	listBytes.push_back( isVersion_E ? sizeof(SWR_MODELHeader_E) : (sizeof(SWR_MODELHeader_X) + sizeof(uint32_t)) );	// offset_Section1
	if (!isVersion_E)
	{
	...
	for (TiXmlElement* node_tmp = node_ListS1 ? node_ListS1->FirstChildElement("Section1") : 0; node_tmp; node_tmp = node_tmp->NextSiblingElement("Section1"))
	{
		...
		*//*
		...
		TiXmlElement* node_SWR_MODELSection1_UnknowEnd = node_tmp->FirstChildElement("UnknowEnd");
		if (node_SWR_MODELSection1_UnknowEnd)
		{
			for (size_t j = 0; j < 500; j++)
			{
				node = node_SWR_MODELSection1_UnknowEnd->FirstChildElement("unk_" + std::to_string(j));
				if (!node)
					break;
...
|                             import_Xml			                             |
\-------------------------------------------------------------------------------*
bool Swr_Model::import_Xml(TiXmlElement* rootNode)
{
	Reset();
	...
	{
		mListTextureUnit.push_back(new Swr_Model_TextureUnit());
	...
	{
		mListPart.push_back(Swr_Model_Part());
...
void Swr_Model::save_Xml(string filename, bool show_error)
{
	...
	emd->name = LibXenoverse::nameFromFilenameNoExtension(filename);
	LibXenoverse::EMM* emm = new LibXenoverse::EMM();
	emm->name = emd->name;
	EMMMaterial* emmMaterial = new EMMMaterial("Default");
	emmMaterial->shaderProgramName = "White";
	emmMaterial->parameters.push_back(new EMMParameter("BackFace", 0x10001, 0, 1));
	emmMaterial->parameters.push_back(new EMMParameter("NoEdge", 0x10001, 0, 1));
	emm->materials.push_back(emmMaterial);

	//test Todo remove
	emmMaterial = new EMMMaterial("Type3");
	emmMaterial->shaderProgramName = "Red";
	emmMaterial->parameters.push_back(new EMMParameter("BackFace", 0x10001, 0, 1));
	emmMaterial->parameters.push_back(new EMMParameter("NoEdge", 0x10001, 0, 1));
	emm->materials.push_back(emmMaterial);
	emmMaterial = new EMMMaterial("Type4");
	emmMaterial->shaderProgramName = "Green";
	emmMaterial->parameters.push_back(new EMMParameter("BackFace", 0x10001, 0, 1));
	emmMaterial->parameters.push_back(new EMMParameter("NoEdge", 0x10001, 0, 1));
	emm->materials.push_back(emmMaterial);
	emmMaterial = new EMMMaterial("Type5");
	emmMaterial->shaderProgramName = "Blue";
	emmMaterial->parameters.push_back(new EMMParameter("BackFace", 0x10001, 0, 1));
	emmMaterial->parameters.push_back(new EMMParameter("NoEdge", 0x10001, 0, 1));
	emm->materials.push_back(emmMaterial);
	...
	//for collisions
	LibXenoverse::EMD* emd_collision = new LibXenoverse::EMD();
	emd_collision->name = LibXenoverse::nameFromFilenameNoExtension(filename);
	LibXenoverse::EMM* emm_collision = new LibXenoverse::EMM();
	emm_collision->name = emd_collision->name;
	emmMaterial = new EMMMaterial("Default_Collision");
	emmMaterial->shaderProgramName = "Grey";
	emmMaterial->parameters.push_back(new EMMParameter("BackFace", 0x10001, 0, 1));
	emmMaterial->parameters.push_back(new EMMParameter("NoEdge", 0x10001, 0, 1));
	emm_collision->materials.push_back(emmMaterial);
	//same Esk
	...
	write_Xml(root, buf, size, emd, emm, esk, ean, emd_collision);
	...
	emd->save(filename + ".emd");
	emm->save(filename + ".emm");
	esk->save(filename + ".esk");
	ean->save(filename + ".ean");
	
	emd_collision->save(filename + "collision.emd");
	emm_collision->save(filename + "collision.emm");
	esk->save(filename + "collision.esk");
	ean->save(filename + "collision.ean");

	delete emd;
	delete emm;
	delete esk;
	delete ean;
	delete emd_collision;
	delete emm_collision;
...
void Swr_Model::write_Xml(TiXmlElement *parent, const uint8_t *buf, size_t size, LibXenoverse::EMD* emd, LibXenoverse::EMM* emm, LibXenoverse::ESK* esk, LibXenoverse::EAN* ean, LibXenoverse::EMD* emd_collision)
{
	...
	EskTreeNode* eskRootNode = esk->getTreeOrganisation();
	...
	for (size_t i = 0; i < nbModels; i++)
	{
		...
		//node_model->LinkEndChild(node_listSection1);					//test Todo uncomment
		...
		EskTreeNode* eskModelNode = new EskTreeNode(new ESKBone(emd->name + "_" + std::to_string(i)), esk->getBones().size(), eskRootNode);
		esk->addBone(eskModelNode->mBone);
		eskRootNode->mChildren.push_back(eskModelNode);
		//test general scale to be usable (vertex position are int16), and a rotation for Y UpAxis for Emd space.
		eskModelNode->mBone->skinning_matrix[4] = 0.707f;				//QuatW
		eskModelNode->mBone->skinning_matrix[5] = 0.0f;					//QuatX
		eskModelNode->mBone->skinning_matrix[6] = 0.0f;					//QuatY
		eskModelNode->mBone->skinning_matrix[7] = -0.707f;				//QuatZ
		/*
		eskModelNode->mBone->skinning_matrix[8]  = 1.0f / 100.0f;				//scaleX
		eskModelNode->mBone->skinning_matrix[9]  = 1.0f / 100.0f;				//scaleY
		eskModelNode->mBone->skinning_matrix[10] = 1.0f / 100.0f;				//scaleZ
		eskModelNode->mBone->skinning_matrix[11] = 1.0f;						//scaleW
		*/
		...
		{	
			...
			for (size_t j = 0; j<listRecusiveAltN.size(); j++)
			{
				...
				EskTreeNode* eskNode = new EskTreeNode(new ESKBone(emd->name + "_" + std::to_string(i) + "__" + uniqueId_str), esk->getBones().size(), listRecusiveAltN_ParentBone.at(j));
				ESKBone* eskBone = eskNode->mBone;
				esk->addBone(eskBone);
				listRecusiveAltN_ParentBone.at(j)->mChildren.push_back(eskNode);


				listAltN_offset.push_back(offset);
				listAltN_BonesName.push_back(eskNode->mBone->getName());
				...
				case 0x3064:
				{
					...
					emdModel->name = emd->name + "_" + std::to_string(i) + "__" + uniqueId_str;

					EMDMesh* emdMesh = new EMDMesh();
					emdModel->meshes.push_back(emdMesh);
					emdMesh->name = emdModel->name;
					emdMesh->aabb_min_x = val_float(section->minX);
					emdMesh->aabb_min_y = val_float(section->minY);
					emdMesh->aabb_min_z = val_float(section->minZ);
					emdMesh->aabb_max_x = val_float(section->maxX);
					emdMesh->aabb_max_y = val_float(section->maxY);
					emdMesh->aabb_max_z = val_float(section->maxZ);
					emdMesh->aabb_center_x = (emdMesh->aabb_max_x + emdMesh->aabb_min_x) / 2.0f;
					emdMesh->aabb_center_y = (emdMesh->aabb_max_y + emdMesh->aabb_min_y) / 2.0f;
					emdMesh->aabb_center_z = (emdMesh->aabb_max_z + emdMesh->aabb_min_z) / 2.0f;
					



					//same for collision
					EMDModel* emdModel_collision = new EMDModel();
					emd_collision->models.push_back(emdModel_collision);
					emdModel_collision->name = emd_collision->name + "_" + std::to_string(i) + "__" + uniqueId_str;

					EMDMesh* emdMesh_collision = new EMDMesh();
					emdModel_collision->meshes.push_back(emdMesh_collision);
					emdMesh_collision->name = emdModel_collision->name;
					emdMesh_collision->aabb_min_x = val_float(section->minX);
					emdMesh_collision->aabb_min_y = val_float(section->minY);
					emdMesh_collision->aabb_min_z = val_float(section->minZ);
					emdMesh_collision->aabb_max_x = val_float(section->maxX);
					emdMesh_collision->aabb_max_y = val_float(section->maxY);
					emdMesh_collision->aabb_max_z = val_float(section->maxZ);
					emdMesh_collision->aabb_center_x = (emdMesh_collision->aabb_max_x + emdMesh_collision->aabb_min_x) / 2.0f;
					emdMesh_collision->aabb_center_y = (emdMesh_collision->aabb_max_y + emdMesh_collision->aabb_min_y) / 2.0f;
					emdMesh_collision->aabb_center_z = (emdMesh_collision->aabb_max_z + emdMesh_collision->aabb_min_z) / 2.0f;
					...
					for (size_t k = 0; k < hdr_AltN->nb_Childs; k++)
					{
						...
						//if ((section3->nbElementV52 == 0) || (section3->nbElementV44 == 0))		//test Todo remove
						//	continue;
						...
						{
							//strangely , it's could have a Section90 without Section44 ... Todo see what it's mean.
						...
						//--------------------------------  Visual Geometry
						EMDSubmesh* emdSubMesh = 0;
						EMDSubmesh* emdSubMesh_Collision = 0;
						
						if (section3->offset_unk52)
						//if ((section3->offset_unk52)&& (emdModel->name + "_submesh_" + std::to_string(k) == "out_modelblock_0__232_submesh_0"))
						{
							...
							//if (!XMLTEST_SoKeepSmaller)
							node_listSection52->SetAttribute("uniqueName", emdSubMesh->name);
						...
						if (section3->offset_unk44)
						{
							...
							for (size_t m = 0; m < nbGroup; m++)
							{
								...
								for (size_t n = 0; n < nbVertex; n++)
								{
									...
									vertex.pos_x = (float)(int16_t)val16(values_u16[valueIndex++]);
									vertex.pos_y = (float)(int16_t)val16(values_u16[valueIndex++]);
									vertex.pos_z = (float)(int16_t)val16(values_u16[valueIndex++]);
									emdSubMesh_Collision->vertices.push_back(vertex);

									//if (offset == 0x1f4)
									//	valueIndex += 3;							//test hypothese normals after position (with normal null, but ...) Todo remove

									node = new TiXmlElement("Vertex");
									node->SetDoubleAttribute("posX", vertex.pos_x);
									node->SetDoubleAttribute("posY", vertex.pos_y);
									node->SetDoubleAttribute("posZ", vertex.pos_z);
						...
						if (section3->offset_section4)
						{
							...
							//test Todo remove:
							//if(emdSubMesh)
							//	emdSubMesh->name = (section3->typeMode == 3) ? "Type3" : ((section3->typeMode == 4) ? "Type4" : "Type5");
				...
				case 0x5066:
				{
					...
					//for model5 , there is only one : on the root.
				}
				break;
				...
				if (eskBone->skinning_matrix[8] * eskBone->skinning_matrix[9] * eskBone->skinning_matrix[10] * eskBone->skinning_matrix[11] == 0)	//Test todo remove
				{
					int aa = 42;
				}
		...
		esk->setTreeOrganisation(eskRootNode);


		//-----------------------  Anim :

		ean->skeleton = new ESK();
		ean->skeleton->clone(esk);
		EANAnimation ean_anim(ean);
		ean_anim.name = "Animation";
		...
		for (size_t inc_Anim = 0; inc_Anim < nbAnim; inc_Anim++)
		{
			...
			if (hdr_anim->offset_values)
			{
				...
				if (nbComponents != 0)
				{
					...
					LibXenoverse::EANAnimationNode* ean_animNode = 0;
					if (isfound != (size_t)-1)
					{
		...
		ean->animations.push_back(ean_anim);
	}
